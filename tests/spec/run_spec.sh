...
assert_contains "$ROOT/.github/workflows/build.yml" "verify-release-performance-evidence.py --commit HEAD" "build workflow verifies version-bump evidence"
assert_contains "$ROOT/.github/workflows/deb.yml" "dpkg-buildpackage -us -uc -b" "deb workflow builds binary package"
assert_contains "$ROOT/.github/workflows/deb.yml" 'verify-release-performance-evidence.py --commit HEAD --tag "${RELEASE_TAG}"' "deb workflow verifies tagged release evidence"
